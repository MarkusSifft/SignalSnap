...
# ==================== c4 for combinations =================================
    def c4(self, a_w1, a_w2, a_w3, a_w4):
        ...
        if self.config.coherent:
            sum_11c22c = af.matmulNT(a_w1 * a_w2_conj, a_w3 * a_w4_conj).T
            ...
            xyzw = af.matmulNT(a_w1_mean * a_w2_conj_mean, a_w3_mean * a_w4_conj_mean).T
            xyzw_mean = mean(xyzw, dim=2)
            
            xy_mean = mean(a_w1_mean * a_w2_conj_mean, dim=2)
            zw_mean = mean(a_w3_mean * a_w4_conj_mean, dim=2)
            xy_zw_mean = af.matmulNT(xy_mean, zw_mean).T

            xz_mean = mean(af.matmulNT(a_w1_mean, a_w3_mean).T, dim=2)
            yw_mean = mean(af.matmulNT(a_w2_conj_mean, a_w4_conj_mean).T, dim=2)
            xz_yw_mean = xz_mean * yw_mean

            xw_mean = mean(af.matmulNT(a_w1_mean, a_w4_conj_mean).T, dim=2)
            yz_mean = mean(af.matmulNT(a_w2_conj_mean, a_w3_mean).T, dim=2)
...
# ======================================================================================
    ...
    def c4_old(self, a_w, a_w2, a_w3, a_w4):
        """
        Calculation of c4 for trispectrum based on equation 60 in arXiv:1904.12154.
        ...
        m = self.config.m
        x = a_w
        y = conj(a_w2) #y a_w_conj
        z = a_w3
        w = conj(a_w4) #w a_w_conj_corr

        ones = to_gpu(np.ones_like(x.to_ndarray()[:, :, 0]))

        sum_11c22c = af.matmulNT(x * y, z * w)  # d1
        sum_11c22c_m = mean(sum_11c22c, dim=2)
        ...
        if self.config.coherent:
            ...
            sum_11c2 = af.matmulNT(x * y, z)  # d2
            sum_11c2_m = mean(sum_11c2, dim=2)
            sum_122c = af.matmulNT(x, y * w)  # d3
            sum_122c_m = mean(sum_122c, dim=2)
            sum_1c22c = af.matmulNT(y, z * w)  # d4
            sum_1c22c_m = mean(sum_1c22c, dim=2)
            sum_11c2c = af.matmulNT(x * y, w)  # d5
            ...
            sum_12c = af.matmulNT(x, w)  # d7
            sum_12c_m = mean(sum_12c, dim=2)
            sum_1c2 = af.matmulNT(y, z)  # d8
            sum_1c2_m = mean(sum_1c2, dim=2)

            sum_12 = af.matmulNT(x, z)  # d9
            sum_12_m = mean(sum_12, dim=2)
            sum_1c2c = af.matmulNT(y, w)  # d9
            sum_1c2c_m = mean(sum_1c2c, dim=2)

            sum_1_m = mean(x, dim=2)  # d10
            sum_1c_m = mean(y, dim=2)  # d11
            sum_2_m = mean(z, dim=2)  # d10
            sum_2c_m = mean(w, dim=2)  # d11
        ...
        if not self.config.turbo_mode:
            ...
            if self.err_counter[order] % self.config.m_var == 0:
                ...
                if order == self.orders[0]:
                    ...
                    #print(self.number_of_error_estimates)
    ...
    def fourier_coeffs_to_spectra(self, orders, a_w_all_gpu, f_max_ind, f_min_ind,
        ...
        for order in orders:
            if order == 1:
                ...
                if self.config.corr_data is not None and self.config.data3 is not None:
                    ...
                    if self.config.combination3 == '2_122':
                        ...
                        t1 = self.calc_a_w3(t0, f_max_ind, self.config.m, self.a_w3_init, self.indi, self.config.backend)
                        a_w3 = to_gpu(t1)
                        single_spectrum = self.c3(a_w1, a_w1, a_w3) / (self.config.delta_t * (single_window ** order).sum())
                    
                    elif self.config.combination3 == '2_212':
                        print('Using 212 combination')
                        a_w1 = af.lookup(a_w_all_corr, af.Array(list(range(f_min_ind, f_max_ind//2))), dim=0)
                        a_w2 = af.lookup(a_w_all_gpu, af.Array(list(range(f_min_ind, f_max_ind//2))), dim=0)
                        t0 = a_w_all_corr.to_ndarray()
                        if self.config.full_bispectrum:
                            a_w1 = af.join(0, af.conjg(af.flip(a_w1[1:], 0)), a_w1)
                            t0 = np.concatenate((t0, np.conj(t0[:0:-1])))
                ...
                if self.config.corr_data is not None and self.config.data3 is not None and self.config.data4 is not None:
                    if self.config.combination4 == '4_1234':
                        ...
                        a_w = af.lookup(a_w_all_gpu, af.Array(list(range(f_min_ind, f_max_ind))), dim=0)
                        a_w2 = af.lookup(a_w3_all_gpu, af.Array(list(range(f_min_ind, f_max_ind))), dim=0)
                        a_w3 = af.lookup(a_w_all_corr, af.Array(list(range(f_min_ind, f_max_ind))), dim=0)
                        a_w4 = af.lookup(a_w4_all_gpu, af.Array(list(range(f_min_ind, f_max_ind))), dim=0)
                        ...
                        a_w = af.lookup(a_w_all_gpu, af.Array(list(range(f_min_ind, f_max_ind))), dim=0)
                        a_w2 = af.lookup(a_w4_all_gpu, af.Array(list(range(f_min_ind, f_max_ind))), dim=0)
                        a_w3 = af.lookup(a_w_all_corr, af.Array(list(range(f_min_ind, f_max_ind))), dim=0)
                        a_w4 = af.lookup(a_w3_all_gpu, af.Array(list(range(f_min_ind, f_max_ind))), dim=0)
                    ...
                    if self.config.combination4 == '3_1233':
                        ...
                        single_spectrum = self.c4(a_w, a_w2, a_w3, a_w2) / (self.config.delta_t * (single_window ** order).sum())
                        ...
                        a_w = af.lookup(a_w_all_corr, af.Array(list(range(f_min_ind, f_max_ind))), dim=0)
                        a_w2 = af.lookup(a_w3_all_gpu, af.Array(list(range(f_min_ind, f_max_ind))), dim=0)
                        a_w3 = af.lookup(a_w_all_gpu, af.Array(list(range(f_min_ind, f_max_ind))), dim=0)
            ...
            self.store_single_spectrum(af.conjg(single_spectrum) , order)
    ...
    def calc_spec(self):
        ...
        #print(single_window)
        window = np.array(m * [single_window]).flatten().reshape((window_points, 1, m), order='F')
        #plt.plot(window[:, 0, 5])
        #print(window)
            ...
            for window_shift in shift_iterator:
                ...
                if self.config.rect_win:
                    ...
                    if self.config.corr_data is not None and self.config.data3 is not None and self.config.data4 is not None:
                        ...
                        a_w3_all_gpu = fft_r2c(window * chunk_a_w3_gpu, dim0=0, scale=self.config.delta_t)
                        a_w4_all_gpu = fft_r2c(window * chunk_a_w4_gpu, dim0=0, scale=self.config.delta_t)
